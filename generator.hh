...
namespace cppcoro
{
  ...
  namespace detail
  {
    ...
    public:
      ...
      template <typename U = T,
                std::enable_if_t<!std::is_rvalue_reference<U>::value,
                                 int> = 0>
      std::suspend_always
    ...
    public:
      ...
      // Iterator needs to be default-constructible to satisfy the
      // Range concept.
      ...
      // Need to provide post-increment operator to implement the
      // 'Range' concept.
  ...
  generator<std::invoke_result_t<
      FUNC &, typename generator<T>::iterator::reference>>
  fmap(FUNC func, generator<T> source)
  {
    for (auto &&value : source)
    {
      co_yield std::invoke(func,
...
namespace cppcoro
{
  ...
  public:
    ...
    public:
      promise_type() noexcept
          : m_value(nullptr), m_exception(nullptr), m_root(this),
            m_parentOrLeaf(this)
      {
      }
    ...
    recursive_generator &
    operator=(recursive_generator &&other) noexcept
    {
      if (this != &other)
      {
        if (m_promise != nullptr)
  ...
  template <typename T>
  void swap(recursive_generator<T> &a,
            recursive_generator<T> &b) noexcept
  {
    a.swap(b);
  }

  // Note: When applying fmap operator to a recursive_generator we
  // just yield a non-recursive generator since we generally won't be
  // using the result in a recursive context.
  template <typename FUNC, typename T>
  generator<std::invoke_result_t<
